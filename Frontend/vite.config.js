...
export default defineConfig(() => {
  ...
  return {
    ...
    server: {
      proxy: {
        '/ws/webrtc': {
          target: isDev ? 'ws://localhost:8000' : 'wss://api.affils.site',
          ws: true,
          changeOrigin: true,
        },
      },
      headers: {
        'Content-Security-Policy': `
          ...
          connect-src 'self'
            ${isDev 
              ? 'ws://localhost:8000 ws://localhost:* http://localhost:* https://localhost:*' 
          ...
          media-src 'self' blob: data: http://localhost:* https://localhost:* https://api.affils.site;
          font-src 'self' data:;
