...
const MusicInfo = React.memo(({ 
  ...
  musicDetails, 
  currentTrack,
  forceUseMusicDetails,
  hasError 
}) => {
  // If no current track and no valid music details, return null or a fallback
  if (!currentTrack && (!musicDetails?.name || forceUseMusicDetails || hasError)) {
  ...
  return (
    <div className="flex items-center space-x-3 min-w-0 flex-1">
      <div className="w-12 h-12 rounded-lg flex-shrink-0 relative overflow-hidden">
        ...
        ) : musicDetails.cover_photo && !forceUseMusicDetails ? (
          <img
